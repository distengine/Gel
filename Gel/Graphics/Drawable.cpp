...
void gel::Drawable::CreateBuffers()
{
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), reinterpret_cast<GLvoid*>(offsetof(Vertex, UV)));
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
}

void gel::Drawable::Draw()
{
	glBindVertexArray(m_Vao);
	if (m_Indices.empty())
